$(document).ready(function(){
    $('#summernote').summernote({
      ...
      $('#summer_textoEmpresa').summernote({
        placeholder: 'Ingrese texto del slider',
        tabsize: 2,
        lang: 'es-ES',
        height: 120,
    ...
    $('#agregarsliderEmpresa').on('submit',function(e){
        e.preventDefault();
        var imagen=$('#imagen');
        var orden=$('#orden').val();
        var pagina=$('#pag').val();
    ...
    $('#actualizarEmpresa').on('submit',function(e){
        e.preventDefault();
        let id=$('#id').val();
        let form= new FormData($('#actualizarEmpresa')[0]);
        let texto= $('#summer_textoEmpresa').summernote('code');
        form.append('texto',texto);
        form.append('_method', 'PUT');
        $.ajax({
            type: "POST",
            url: "actualizarContenido/"+id,
            data: form,
            processData: false,  // tell jQuery not to process the data
            contentType: false,   // tell jQuery not to set contentType
            headers: {
            ...
            success: function (response) {
               swal("Contenido editado","","success");
               setTimeout(function(){ location.reload(); }, 1500);
            },
            error: function(response) {
                console.log(response);
